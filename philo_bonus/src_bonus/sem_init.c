...
/*   Updated: 2023/07/12 22:06:28 by vhovhann         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	semaphores_init(t_philo *philo,  int count_philo)
{
	...
	philo->sem_die = sem_open(SEM_DIE, O_CREAT, S_IRWXU, 1);
	...
	if (philo->sem_die == SEM_FAILED || philo->sem_each_eat == SEM_FAILED || \
		philo->sem_fork == SEM_FAILED || philo->sem_last_eat == SEM_FAILED || philo->sem_write == SEM_FAILED)
	{
		perror("sem_open");
		exit(EXIT_FAILURE);
	}
